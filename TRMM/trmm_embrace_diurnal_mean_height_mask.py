...
gtopo30 = np.load('/nfs/a90/eepdw/Data/Observations/Satellite/GTOPO3_1km_topography/gt30e060n40.npz')

#h = np.where(gtopo30['gtopo30_data']!=-9999, gtopo30['gtopo30_data'], np.NaN)
h = gtopo30['gtopo30_data']
gtopo_lat = gtopo30['lats_gtopo30']
gtopo_lon = gtopo30['lons_gtopo30']

gtopo_lons, gtopo_lats = np.meshgrid(gtopo_lon, gtopo_lat)
...
gtopo_regrid = griddata((gtopo_lats.flatten(), gtopo_lons.flatten()), h.flatten(), (lats_data,lons_data), method='nearest')
...
pcp_dom_temp = pcp_dom[:,la_i_min_pcp:la_i_max_pcp, lo_i_min_pcp:lo_i_max_pcp]
lsm= lsm_regrid[la_i_min_pcp:la_i_max_pcp, lo_i_min_pcp:lo_i_max_pcp]
heights=gtopo_regrid[la_i_min_pcp:la_i_max_pcp, lo_i_min_pcp:lo_i_max_pcp]

heights= np.repeat(np.expand_dims(heights,axis=0), pcp_dom_temp.shape[0], axis=0) # Add heights mask for every time

pcp_dom_2 = np.ma.masked_array(pcp_dom_temp, (heights<=height_max) & (heights>=height_min)) # Make masked array for averaging
...
np.savez("/nfs/a90/eepdw/Data/Observations/Satellite/TRMM/Diurnal/land_trmm_diurnal_average_lat_%s_%s_lon_%s_%s_height_%s_to_%s" % (lat_low,lat_high, lon_low, lon_high, height_min, height_max), mean=mean_l, hour=hour_l)
