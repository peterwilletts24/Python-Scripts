...
try:
    regrid_cube_init = iris.load_cube(fr, ('%s' % cube_names[0])  & glob_tc)
except iris.exceptions.ConstraintMismatchError:
    regrid_cube_init = iris.load_cube(fr, ('%s' % cube_names2[0])  & glob_tc)
del glob_load
...
for experiment_id in experiment_ids:
 ...
 try:
     os.remove('%s%s/%s/%s_mean_by_hour_regrid.pp' % (pp_file_path, expmin1, experiment_id, diag))
 except OSError:
     print '%s%s/%s/%s_mean_by_hour_regrid.pp NOT REMOVED' % (pp_file_path, expmin1, experiment_id, diag)
     pass 
 ...
 iris.save((mean),'%s%s/%s/%s_mean_by_hour_regrid.pp' % (pp_file_path, expmin1, experiment_id, diag), append=True)
