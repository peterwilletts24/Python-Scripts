...
pp_filerain = 'rain_mean'

pp_file='Net_Surface_Water_Balance'
...
def main():
 ...
 for experiment_id in experiment_ids:
  ...
  pfilerain = '/nfs/a90/eepdw/Data/EMBRACE/Mean_State/pp_files/%s/%s/%s.pp' % (expmin1, experiment_id, pp_filerain)
  ...
  pcuberain = iris.load_cube(pfilerain)
  pcube3234 = iris.load_cube(pfile3234)

  print latent_heat_of_vapourisation
  
  evap_rate = (pcube3234/latent_heat_of_vapourisation)*convert_to_kg_m2_day
  evap_rate.units = None
  
  rain_daily=pcuberain*convert_to_kg_m2_day

  rain_daily.units = None
  waterbalance = evap_rate - rain_daily

  pcubetotal = waterbalance
  print pcubetotal
     #print pc
 ...
 # Get min and max latitude/longitude and unrotate  to get min/max corners to crop plot automatically - otherwise end with blank bits on the edges 
  ...
  cont = iplt.contourf(pcubetotal, clevs, cmap=cmap, norm=norm, extend='both')
