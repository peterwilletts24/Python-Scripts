...
file_name = 'TimeVar_408_on_p_levs_mean_by_day_land_domain_constrain__and_oro_not_greater_than_data_monsoon_trough.npz'
    
diag_title = 'Monsoon Trough Minimum 925 hPa Height'
area='monsoon_trough'
...
def main():
  for type_of_plot in types_of_plot:
    ...
    if type_of_plot=='8_and_12_km_plus':
        ...
        experiment_ids_e = ['dklyu', 'dklwu', 'dkbhu', 'dkmgw'] # Explicit
    ...
    if type_of_plot=='all':
        ...
        experiment_ids_e = ['dklwu', 'dklyu', 'djzns', 'dkbhu', 'djznu', 'dkhgu', 'dkmgw'] # Most of Explicit
 ...
 #cgen = (cmap(1.*i/NUM_COLORS) for i in range(NUM_COLORS))
    ...
    for ls in ['land']:
     ...
     era_npz = np.load('/nfs/a90/eepdw/Data/Era_Interim/Era_interim_TimeVar_on_p_levs_mean_by_day_land_domain_'\
     'constrain__and_oro_not_greater_than_data_monsoon_trough_%s.npz' % p_lev)

     l, = plt.plot_date(ConvertTimeStampStringToDatetime(era_npz['time_coords'], '%Y-%m-%d'), era_npz['data']/10
                   , label='ERA-I', linewidth=1, linestyle='-', marker='', fmt='', color='#262626')

     legendEntries.append(l)
     legendtext.append('ERA-I')
