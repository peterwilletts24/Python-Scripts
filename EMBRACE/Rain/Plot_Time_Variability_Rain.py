...
def main():
 ...
 #cgen = (cmap(1.*i/NUM_COLORS) for i in range(NUM_COLORS))
  ...
  # Change the legend label colors to almost black
     ...
     plt.savefig('%s/EMBRACE_%s_%s_%s_notitle.png' % (save_dir,  pp_filenodot, ls, type_of_plot),
     ...
     plt.savefig('%s/EMBRACE_%s_%s_%s.png' % (save_dir, pp_filenodot, ls, type_of_plot),
                 format='png', bbox_inches='tight')
     #plt.close()
