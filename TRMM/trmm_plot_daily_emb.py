...
sum_dom, latitude_domsingle, longitude_domsingle, dates_list, hours= pickle.load(open('/nfs/a90/eepdw/Data/Saved_data/TRMM/trmm_emb_pcpmean_by_day.p', 'rb'))
...
#cbar = m.colorbar(cs,location='bottom',pad="5%")
    ...
    cbar.set_label('mm/h')
     ...
     #   print hours[i]
    plt.savefig('/nfs/a90/eepdw/Figures/TRMM/TRMM_daily_mean_EMBRACE_period_%s_notitle.png' % (dates_list[i].strftime('%Y_%m_%d')), format='png', bbox_inches='tight')

    plt.title('TRMM Rainfall Retrieval Mean for %s (UTC)' % (dates_list[i].strftime('%Y-%m-%d')), fontsize=16, color='#262626')

    plt.savefig('/nfs/a90/eepdw/Figures/TRMM/TRMM_daily_mean_EMBRACE_period_%s_title.png' % (dates_list[i].strftime('%Y_%m_%d')), format='png', bbox_inches='tight')

    plt.title('%s (UTC) ' % (dates_list[i].strftime('%Y-%m-%d')), fontsize=16, color='#262626')
    plt.savefig('/nfs/a90/eepdw/Figures/TRMM/TRMM_daily_mean_EMBRACE_period_%s_short_title.png' % (dates_list[i].strftime('%Y_%m_%d')), format='png', bbox_inches='tight')
