...
# Calculate weights
        ...
        for s in ([0,1]):
             ...
             mean = masked_iris_cube.aggregated_by('hour', iris.analysis.MEAN)
             ...
             if s == 0:
                 ...
                 np.savez('%s%s/%s/%s_sea_rainfall_np_domain_constrain_hourly' % (pp_file_path, expmin1, experiment_id, diag), 
                     data=mean2.data.data, time_coords=mean2.coord('time').points)
        
             if s == 1:
                 # Areas of land
                 np.savez('%s%s/%s/%s_land_rainfall_np_domain_constrain_hourly' % (pp_file_path, expmin1, experiment_id, diag), 
                    data=mean2.data.data, time_coords=mean2.coord('time').points)      
