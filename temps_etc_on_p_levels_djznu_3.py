...
def main():
 ...
 for experiment_id in experiment_ids:
  ...
  try:
      ...
      fname_heights = '/projects/cascade/pwille/moose_retrievals/%s/%s/15101_2.pp'% (expmin1, experiment_id)
      ...
      load_name_i='/408_pressure_levels_interp_pressure_%s_3' % experiment_id

      save_name_temp='/temp_pressure_levels_interp_%s_1to8' % experiment_id
      save_name_sp_hum='/sp_hum_pressure_levels_interp_%s_1to8' % experiment_id
        ...
        for t, time_cube in enumerate(temp_cube.slices(['model_level_number', 'grid_latitude', 'grid_longitude'])):
                ...
                for f in range(lat_len):
                 ...
                 for y in range(lon_len):
                     #tc = time_cube[:,f,y].data  # Individual 
                     print tc
                     i_func_t = InterpolatedUnivariateSpline(heights[:,f,y], tc[:,y])
                     sys.stdout.flush()
                     print i_func_t
                     sys.stdout.flush()
