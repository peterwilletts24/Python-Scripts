...
def main():
 ...
 for experiment_id in experiment_ids:
  for p in pressure_levels:
   ...
   pfile = '/nfs/a90/eepdw/Data/EMBRACE/Mean_State/pp_files/%s/%s/%s_%s_%s.pp' % (expmin1, experiment_id, pp_file, p, type)
 ...
 # Get min and max latitude/longitude and unrotate  to get min/max corners to crop plot automatically - otherwise end with blank bits on the edges
  ...
  #  plt.savefig('%s%s/%s/%s_%s_.png' % (save_path, experiment_id, pp_file, experiment_id, pp_file, p), format='png', bbox_inches='tight')
