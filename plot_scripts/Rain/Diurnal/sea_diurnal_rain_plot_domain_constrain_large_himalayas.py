...
trmm_file = "trmm_diurnal_average_himalayas_polygon.npz" 
 ...
 for ls in ['land']:
  ...
  for c, experiment_id in enumerate(experiment_ids_p):
   ...
   try:
      ...
      plotnp = np.load('%s/%s/%s/%s_%s_rainfall_diurnal_np_domain_constrain_himalayas.npy' % (top_dir, expmin1, experiment_id, pp_file, ls))
...
# Bit of formatting 
  ...
  plt.savefig('/nfs/a90/eepdw/Figures/EMBRACE/Diurnal/%s_%s_latlon_himalayas_notitle_largeonly.png' % (pp_filenodot, ls), format='png', bbox_inches='tight')

  plt.title('\n'.join(wrap('%s' % (t.title()), 1000,replace_whitespace=False)), fontsize=16)
  #plt.show()
  
  plt.savefig('/nfs/a90/eepdw/Figures/EMBRACE/Diurnal/%s_%s_latlon_himalayas__largeonly.png' % (pp_filenodot, ls), format='png', bbox_inches='tight')
  plt.close()
