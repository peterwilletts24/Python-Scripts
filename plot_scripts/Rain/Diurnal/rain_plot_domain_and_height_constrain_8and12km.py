...
filename = 'avg.5216_land_rainfall_diurnal_np_domain_constrain_lat_%s-%s_lon-%s-%s_heights_%s-%s.npy' % (lat_min,lat_max, lon_min_rot, lon_max_rot,height_min2, height_max2)
trmm_file = "trmm_diurnal_average_lat_%s_%s_lon_%s_%s_height_%s_to_%s.npz" % (lat_min,lat_maxtrmm, lon_min, lon_max,height_min, height_max)
#############
 ...
 for ls in ['land']:
  ...
  for c, experiment_id in enumerate(experiment_ids_p):
   ...
   if (experiment_id=='djznu'):
          ...
          linestylez='-'
   try:
      plotnp = np.load('%s/%s/%s/%s_%s_rainfall_diurnal_np_domain_constrain_lat_%s-%s_lon-%s-%s_heights_%s-%s.npy' % (top_dir, expmin1, experiment_id, pp_file, ls, lat_min, lat_max, lon_min_rot, lon_max_rot, height_min, height_max))
...
# Bit of formatting 
  ...
  filenamesave='%s_%s_lat_%s-%s_lon_%s-%s_height_%s-%s' %(pp_file, ls, lat_min, lat_max, lon_min, lon_max, height_min, height_max)

  filename_nodot= filenamesave.replace(".", "")

  plt.savefig('/nfs/a90/eepdw/Figures/EMBRACE/Diurnal/%s_notitle_8and12kmonly.png' %filename_nodot, format='png', bbox_inches='tight')
  
  plt.title('\n'.join(wrap('%s' % (t.title()), 1000,replace_whitespace=False)), fontsize=16)
  #plt.show()

  plt.savefig('/nfs/a90/eepdw/Figures/EMBRACE/Diurnal/%s_8and12kmonly.png' % filename_nodot, format='png', bbox_inches='tight')
