...
def main():
 ...
 # Get min and max latitude/longitude and unrotate  to get min/max corners to crop plot automatically - otherwise end with blank bits on the edges 
  ...
  #ax.drawparallels(np.arange(-80,81,10),labels=[1,1,0,0])
  #ax.drawmeridians(np.arange(0,360,10),labels=[0,0,0,1])
  ...
  #cbar.set_label('') 
  cbar.set_label(pc.units)
  cbar.set_ticks(np.arange(min_contour, max_contour+tick_interval,tick_interval))
  ticks = (np.arange(min_contour, max_contour+tick_interval,tick_interval))
  cbar.set_ticklabels(['%d' % i for i in ticks])
  main_title=pc.standard_name.title().replace('_',' ')
  model_info=re.sub('(.{65}. )', '\\1\n', str(model_name_convert_title.main(experiment_id)), 0, re.DOTALL)
  model_info = re.sub(r'[(\']', ' ', model_info)
  model_info = re.sub(r'[\',)]', ' ', model_info)
  print model_info
  plt.title('\n'.join(wrap('%s\n%s' % (main_title, model_info), 1000,replace_whitespace=False)), fontsize=12, wieght='normal', color='#262626')
  ...
  #plt.savefig('/home/pwille/figures/%s/%s/%s%s_%s.png' % (experiment_id, pp_file, pc.standard_name.title(), experiment_id, pp_file), format='png', bbox_inches='tight')
  #plt.close()
