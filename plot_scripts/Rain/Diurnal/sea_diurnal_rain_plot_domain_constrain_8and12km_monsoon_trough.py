...
lon_max = 80
lon_min = 64.115   #  For the monsoon trough plots, these numbers don't mean anything they are just 
lat_max= 5         # to get the right filename
lat_min=-10
...
# Bit of formatting 
  ...
  plt.savefig('/nfs/a90/eepdw/Figures/EMBRACE/Diurnal/%s_%s_latlon_monsoon_trough_notitle_8and12kmonly.png' % (pp_filenodot, ls), format='png', bbox_inches='tight')

  plt.title('\n'.join(wrap('%s' % (t.title()), 1000,replace_whitespace=False)), fontsize=16)
  #plt.show()
  
  plt.savefig('/nfs/a90/eepdw/Figures/EMBRACE/Diurnal/%s_%s_latlon_monsoon_trough_8and12kmonly.png' % (pp_filenodot, ls), format='png', bbox_inches='tight')
  plt.close()
