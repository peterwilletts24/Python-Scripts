...
file_name = 'avg.5216_land_rainfall_TimeVar_np_domain_constrain.npz'
...
#trmm_dir = '/nfs/a90/eepdw/Data/Saved_data/TRMM/'
#trmm_file = "trmm_emb_pcpmean_time_var.p"
# trmm_load = pickle.load(open('/nfs/a90/eepdw/Data/Saved_data/TRMM/trmm_emb_pcpmean_time_va_dkbhu_domain.p'))

# trmm_datetimes = ConvertTimeStampStringToDatetime(trmm_load[3], '%Y-%m-%d %H')   

# time_arg_sort = np.argsort(trmm_datetimes)

# trmm_datetimes_sort =  trmm_datetimes[time_arg_sort]
# trmm_data_sort =  trmm_load[0][time_arg_sort]
#pdb.set_trace()
...
def main():
 ...
 #cgen = (cmap(1.*i/NUM_COLORS) for i in range(NUM_COLORS))
    ...
    for ls in ['land']:
     ...
     #plt.plot_date(trmm_datetimes_sort, trmm_data_sort, linestyle='-', color= '#262626', linewidth=1., marker='')
     ...
     #      linewidth=1.6
         ...
         #  plotnp = np.load('%s/%s/%s/%s' % (top_dir, expmin1, experiment_id, file_name))
          ...
          plotnp = np.load('/nfs/a90/eepdw/Data/EMBRACE/dkly/dklyu/divergence_925.0_TimeVar_np_domain_constrain.npz')

          time_arg_sort = np.argsort(plotnp['time_coords'].flatten())

             #time_sort = plotnp[1][hour_arg_sort]
          data_sort =  np.rollaxis(plotnp['data'], axis=1).flatten()[time_arg_sort]

          #minute_min,hour_min = math.modf(plotnp['time_coords'].points.min()) 
          #minute_max,hour_max = math.modf(plotnp['time_coords'].points.max()) 

          plot_dates = ConvertHoursSince1970ToDatetime(plotnp['time_coords'].flatten[time_arg_sort])
  ...
  # Change the legend label colors to almost black
     ...
     for c, experiment_id in enumerate(experiment_ids_e):
            ...
            try:
             ...
             l, = plt.plot_date(plot_dates, data_sort*3600, label=model_name_convert_legend.main(experiment_id), \
                                linewidth=linewidth, linestyle=linestylez, marker='', markersize=2, fmt='', color=colour)
             #else:
