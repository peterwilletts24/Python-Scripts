...
# Plot diagnostics, model and pressure levels etc. to plot on for looping through
 ...
 #experiment_ids = ['dkmbq', 'djzns', 'dkbhu']
...
## Load global wind and orography
 ...
 fw_global = '/nfs/a90/eepdw/Mean_State_Plot_Data/pp_files/dkmb/dkmbq/30201_mean.pp'
 fo_global = '/nfs/a90/eepdw/Mean_State_Plot_Data/pp_files/dkmb/dkmbq/33.pp'
...
# Unrotate global winds
 ...
 if isinstance(cs_glob, iris.coord_systems.RotatedGeogCS):
        print ' Global Model - Winds - dkmbq - Unrotate pole %s' % cs_glob
...
# Unrotate global model
 ...
 if isinstance(cs_glob_oro, iris.coord_systems.RotatedGeogCS):
        print ' Global Model - Orography - dkmbq - Unrotate pole %s - Winds and other diagnostics may have different number of grid points' % cs_glob_oro
...
####################  Load global heights and temp/sp_hum #####################
 ...
 f_glob_h = '/nfs/a90/eepdw/Mean_State_Plot_Data/Mean_Heights_Temps_etc/408_pressure_levels_interp_pressure_dkmbq_%s' % (plot_type)
...
## Loop through experiment id's ######################################################
 ...
 for  pl in plot_diags:
  ...
  f_glob_d = '/nfs/a90/eepdw/Mean_State_Plot_Data/Mean_Heights_Temps_etc/%s_pressure_levels_interp_dkmbq_%s' % (plot_diag, plot_type)
...
Basemap(llcrnrlon=lon_low,llcrnrlat=lat_low,urcrnrlon=lon_high,urcrnrlat=lat_high, rsphere = 6371229)
        ...
        plt.savefig('/nfs/a90/eepdw/Mean_State_Plot_Data/Figures/%s/%s/geop_height_difference_8km_%shPa_%s_%s.png' % (experiment_id, plot_diag, p, experiment_id, plot_diag), format='png', bbox_inches='tight')
        plt.cla()
        plt.clf()
